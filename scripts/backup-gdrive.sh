...
# Cron:  0 3 * * * /opt/fashionecom/scripts/backup-gdrive.sh
# ──────────────────────────────────────────
...
# Fallback /opt/fashionecom/.env (production layout)
if [ -z "${GDRIVE_ENABLED:-}" ] && [ -f /opt/fashionecom/.env ]; then
  set -a
  # shellcheck disable=SC1091
  . /opt/fashionecom/.env
  set +a
...
# Determine log file — preferred /var/log/fashionecom, fallback ./logs/
LOG_DIR="/var/log/fashionecom"
