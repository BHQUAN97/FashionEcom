...
# Run via cron: 0 2 * * * /opt/fashionecom/scripts/backup-mysql.sh
# ──────────────────────────────────────────
...
BACKUP_DIR="${BACKUP_DIR:-/opt/fashionecom/backups/mysql}"
RETENTION_DAYS="${RETENTION_DAYS:-7}"
DB_CONTAINER="${DB_CONTAINER:-fashionecom-mysql}"
DB_NAME="${DB_NAME:-fashion_ecom}"
DB_USER="${DB_USER:-fashionecom}"
LOG_FILE="${LOG_FILE:-/var/log/fashionecom/backup.log}"
...
if [ -z "${MYSQL_BACKUP_PASSWORD:-}" ]; then
  if [ -f /opt/fashionecom/.env ]; then
    DB_PASSWORD=$(grep '^LQD_DB_PASSWORD=' /opt/fashionecom/.env | cut -d= -f2- | tr -d '"' | tr -d "'")
  fi
...
if command -v rclone &> /dev/null; then
  REMOTE="${RCLONE_REMOTE:-r2:fashionecom-backups/mysql/}"
