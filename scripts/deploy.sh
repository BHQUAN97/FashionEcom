#!/bin/bash
# ============================================================
# FashionEcom — FULL DEPLOY (first-time) TO VPS
# ============================================================
...
#
# Usage:
#   bash scripts/deploy.sh <vps-ip> [domain]
#   bash scripts/deploy.sh 143.198.217.127 shop.bhquan.store
# ============================================================

set -e

VPS_IP="${1:?Usage: bash scripts/deploy.sh <vps-ip> [domain]}"
...
echo ""
echo "============================================================"
echo "  FashionEcom — Full Deploy (first-time)"
echo "  VPS:    ${VPS_HOST}"
echo "  Domain: ${DOMAIN}"
...
step "4/8 — Tao database fashion_ecom trong shared-mysql + .env"
...
JWT_REFRESH_SEC=$(openssl rand -hex 32)
REVAL_SEC=$(openssl rand -hex 16)

# Lay R2 + Resend keys tu WebPhoto (neu co)
R2_ENDPOINT=$(ssh "${VPS_HOST}" "grep '^R2_ENDPOINT=' ${WEBPHOTO_DIR}/.env | cut -d= -f2-" 2>/dev/null || true)
R2_ACCESS_KEY=$(ssh "${VPS_HOST}" "grep '^R2_ACCESS_KEY=' ${WEBPHOTO_DIR}/.env | cut -d= -f2-" 2>/dev/null || true)
R2_SECRET_KEY=$(ssh "${VPS_HOST}" "grep '^R2_SECRET_KEY=' ${WEBPHOTO_DIR}/.env | cut -d= -f2-" 2>/dev/null || true)
RESEND_KEY=$(ssh "${VPS_HOST}" "grep '^RESEND_API_KEY=' ${WEBPHOTO_DIR}/.env | cut -d= -f2-" 2>/dev/null || true)
...
ssh "${VPS_HOST}" "
  ...
  # Ket noi shared-nginx vao fashionecom network
...
ssh "${VPS_HOST}" "
  ...
  if [ -d '${APP_DIR}/db/changelog' ]; then
    for f in \$(find ${APP_DIR}/db/changelog -name '*.sql' -type f 2>/dev/null | sort); do
      FNAME=\$(basename \"\$f\")
      echo -n \"  \$FNAME ... \"
      docker exec -i shared-mysql mysql -u fashionecom -p'${SHOP_DB_PASS}' fashion_ecom < \"\$f\" 2>&1 && echo 'OK' || echo 'SKIP'
    done
  fi
...
scp "$ROOT_DIR/nginx/conf.d/${DOMAIN}.conf" "${VPS_HOST}:${WEBPHOTO_DIR}/nginx/conf.d/${DOMAIN}.conf" 2>/dev/null || \
...
ssh "${VPS_HOST}" "
  ...
  if ! [ -d /etc/letsencrypt/live/${DOMAIN} ]; then
    ...
    certbot certonly --standalone -d ${DOMAIN} \
      ...
      --post-hook 'docker start shared-nginx' 2>&1 || warn 'SSL cert failed — configure manually'
  else
...
ssh "${VPS_HOST}" "
  ...
  echo 'Health check:'
  curl -sf http://localhost:4000/api/health 2>/dev/null && echo '' || echo 'API not ready yet (may need 10-20s)'
  curl -sf -o /dev/null http://localhost:3000 && echo 'Frontend: OK' || echo 'Frontend not ready yet'
...
echo "============================================================"
echo ""
echo "  https://${DOMAIN}        (FashionEcom Storefront)"
echo "  https://${DOMAIN}/admin  (Admin Panel)"
echo ""
echo "  Update deploy:  bash scripts/update-deploy.sh ${VPS_IP}"
echo "  DB changelog:   bash scripts/db-changelog.sh ${VPS_IP}"
echo "============================================================"
